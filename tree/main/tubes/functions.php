...
function koneksi()
{
  ...
  mysqli_select_db($conn, 'petcare') or die("Database salah!");
  return $conn;
}
...
function hapus($id)
{
  ...
  $petcare = query("SELECT * FROM petcare WHERE id = $id")[0];

  // hapus data gambar
  if ($petcare["gambar"] !== 'nophoto.jpg') {
    unlink('img/' . $petcare["gambar"]);
  }

  mysqli_query($conn, "DELETE FROM petcare WHERE id = $id") or die(mysqli_error($conn));
  return mysqli_affected_rows($conn);
}
